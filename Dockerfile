...
ARG	TMP_DIR=/tmp/app
ARG	CLOUDCLI_REF CLOUDCLI_HEAD
RUN	echo "CLOUDCLI_HEAD=${CLOUDCLI_HEAD}"
RUN	<<-EOF
	mkdir -p ${TMP_DIR}
	git clone --depth 1 --branch ${CLOUDCLI_REF} \
		https://github.com/siteboon/claudecodeui.git ${TMP_DIR}
EOF
WORKDIR	${TMP_DIR}
RUN	<<-EOF
...
RUN	--mount=type=cache,target=/var/cache/apt,sharing=locked \
	--mount=type=cache,target=/var/lib/apt,sharing=locked <<-EOF
	apt-get update
	apt-get --no-install-recommends install -y jq less openssh-client ripgrep unzip vim
EOF
RUN	<<-EOF
